...
class MainWindow(QMainWindow):
    ...
    def refresh_plot_view(self):
        ...
        # Ensure aspect ratio is reasonable for Nyquist (often wildly different scales)
        # self.ax_nyquist.set_aspect('equal') # Sometimes bad if R >> X
    ...
    def on_plot_click(self, event):
        ...
        if event.inaxes == self.ax_mag:
            ...
            self.ax_mag.plot(f_mhz[idx], val, 'ro')
            self.ax_mag.annotate(f"{f_mhz[idx]:.1f} MHz\n{val:.1f} dB", (f_mhz[idx], val),
        ...
        # We can add click handlers for Bode/Nyquist if needed later
